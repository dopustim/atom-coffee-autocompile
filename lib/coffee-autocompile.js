...
import coffeeFallback from 'coffeescript';
...
function render(source, params) {
  ...
  getCoffeeExecutable(path.dirname(inPath)).then(coffee =>
    ...
    showView('error', 'coffeescript:', error.toString())
  );
}
...
function postRenderNoSourcemap(js, {compress}) {
  return !compress ? js : uglifyJS.minify(js).code;
}

function getCoffeeExecutable(basedir) {
  return getModulePath(basedir, 'coffeescript').then(filename => {
    try {
